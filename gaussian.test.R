# data generation.
gaussian.test.generator<-function(n=5000,seed=1){
  set.seed(seed)
  a = data.frame(A = rnorm(n, 1, 1),
                 B = rnorm(n, 2, 3),
                 C = rep(0, n),
                 D = rep(0, n),
                 E = rnorm(n, 3.5, 2),
                 F = rep(0, n),
                 G = rnorm(n, 5, 2))
  a$C = 2 * (a$A + a$B) + rnorm(n, 2, 0.5)
  a$D = 1.5 * a$B + rnorm(n, 6, 0.33)
  a$F = 2 * a$A + a$D + a$E + 1.5 * a$G + rnorm(n, 0, 1)
  a
}
