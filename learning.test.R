# data generation.
learning.test.generator<-function(n=5000, seed=1){
  set.seed(seed)
  
  LV3 = c("a", "b", "c")
  a = sample(LV3, n, prob = rep(1/3, 3), replace = TRUE)
  c = sample(LV3, n, prob = c(0.75, 0.2, 0.05), replace = TRUE)
  f = sample(c("a", "b"), n, prob = rep(1/2, 2), replace = TRUE)
